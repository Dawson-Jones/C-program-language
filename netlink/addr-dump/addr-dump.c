...
int print_linkinfo(struct nlmsghdr *n, void *arg)
{
	...
	if (tb[IFA_ADDRESS]) {
		static char buf[256];
		inet_ntop(AF_INET, tb[IFA_ADDRESS] + 1, buf, sizeof(buf));
		printf("addr: %s\n", buf);
	}
...
int iplink_get(const char *name, __u32 filt_mask, struct rtnl_handle *rth, int index)
{
...
int main(int argc, char const *argv[])
{
	...
	int index = if_nametoindex(filter_dev);
	...
	iplink_get(filter_dev, RTEXT_FILTER_VF, &rth, index);
