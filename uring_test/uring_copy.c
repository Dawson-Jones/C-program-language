...
queue_read(struct io_uring *ring, off_t size, off_t offset)
{
    ...
    data = malloc(size + sizeof(struct io_data));
    if (!data) {
        return 1;
    }
...
queue_write(struct io_uring *ring, struct io_data *data)
{
    ...
    // base 在读的不够满的时候, 有可能发生变化, 重新调整回起点
    data->iov.iov_base = data + 1;
...
copy_file(struct io_uring *ring, off_t insize)
{
    ...
    while (insize || write_left) {
        ...
        while (insize) {
            ...
            if (queue_read(ring, this_size, offset) != 0)
                break;
            
            insize -= this_size;
            offset += this_size;
            reads++;
            // printf("%s(%s:%d) reads: %ld, offset: %ld\n", __func__, __FILE__, __LINE__, reads, offset);
        }
        ...
        // Let's find a least one completion
        got_comp = 0;
        while (write_left) {
            struct io_data *data;
            ...
            if (!cqe) {
                // printf("%s(%s:%d) no completion, don't write now\n", __func__, __FILE__, __LINE__);
                break;
            }
            ...
            if (data->read) {
                ...
                reads--;
                writes++;
                // printf("%s(%s:%d) reads: %ld, writes: %ld\n", __func__, __FILE__, __LINE__, reads, writes);
            } else {
                free(data);
                writes--;
            }
...
int main(int argc, char *argv[])
{
    ...
    if (setup_context(QUEUE_DEPTH, &ring) != 0)
        return 1;

    if (get_file_size(infd, &insize) != 0) 
        return 1;
